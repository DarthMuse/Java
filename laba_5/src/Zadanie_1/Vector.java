...
public class Vector {
    ...
    public boolean ort(int x11, int x12,
        ...
        boolean check = false;
        try {
            Double.parseDouble(Integer.toString(sum));
            check = true;
    ...
    public boolean per(int x11, int x12,
        ...
        boolean check = false;
        try {
            Double.parseDouble(Integer.toString(v1 * v2 * v3 * v4));
            check = true;
        } catch (NumberFormatException e) {
            System.out.println("Ошибка вычисления");
        }

        if (v1 * v2 < 0 && v3 * v4 < 0 && check){
            return true;
        }
        else {
            return false;
    ...
    public int vektorMulti(int ax, int ay,
        ...
        boolean check = false;
        try {
            Double.parseDouble(Integer.toString(ax*by - bx*ay));
            check = true;
        } catch (NumberFormatException e) {
            System.out.println("Ошибка вычисления");
        }
    ...
    public boolean sravn(int x11, int x12,
        ...
        boolean check = false;
        try {
            Double.parseDouble(Double.toString(mod_one * mod_two));
            check = true;
        } catch (NumberFormatException e) {
            System.out.println("Ошибка вычисления");
        }

        if(mod_one == mod_two && check){
            return true;
        }
        else{
            return false;
